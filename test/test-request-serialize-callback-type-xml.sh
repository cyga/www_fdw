...
for inf in "$test_dir/request-serialize-callback-type-xml/"*".in"; do
...
$psql -f "$test_dir/request-serialize-callback-type-xml-indent.sql"

for inf in "$test_dir/request-serialize-callback-type-xml-indent/"*".in"; do
	r=`$psql -tA < "$inf" 2>&1 | grep -v CONTEXT:`
	outf=`echo $inf | sed 's/\.in$/.out/'`
	out=`cat "$outf"`
	in=`cat "$inf"`
